...
class HumanHandoffRequestViewSet(viewsets.ModelViewSet):
    ...
    def _send_client_notification(self, handoff, message):
        ...
        # Si es usuario anónimo, por ahora no enviamos email si no hay user
        # TODO: Soportar notificaciones a anónimos si NotificationService lo permite
        if not user:
            logger.info(
                "Handoff %d es de usuario anónimo, no se puede enviar notificación por ahora",
                handoff.id
            )
            return
        ...
        try:
            ...
            # Registrar delivery (asumimos éxito si no hay excepción, aunque es async)
            message.delivered_at = timezone.now()
            message.delivery_channel = 'notification_service'
            message.save(update_fields=['delivered_at', 'delivery_channel'])
            
            logger.info(
                "Notificación enviada a %s para handoff %d",
                user.email, handoff.id
            )
        except Exception as e:
            logger.error(
                "Error enviando notificación al cliente para handoff %d: %s",
