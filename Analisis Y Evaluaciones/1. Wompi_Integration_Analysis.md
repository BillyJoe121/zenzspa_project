# 📊 ANÁLISIS: Módulo Finances vs Documentación Oficial de Wompi
...
**Fecha:** 2025-11-26
**Versión Wompi API:** 1.2.0 (OAS 3.0)
...
Evaluar el estado actual del módulo `finances` y la centralización de pagos en el proyecto StudioZens, comparándolo con la documentación oficial de Wompi para identificar brechas de implementación, características faltantes y oportunidades de mejora.

---

## 📚 DOCUMENTACIÓN OFICIAL DE WOMPI ANALIZADA

### **Endpoints Principales:**

#### **Transacciones**
- `GET /transactions/{transaction_id}` - Obtener una transacción
- `GET /transactions` - Buscar transacciones
- `POST /transactions` - **Crear una transacción**
- `POST /transactions/{transaction_id}/void` - Anular una transacción

#### **Tokenización**
- `POST /tokens/cards` - Tokenizar una tarjeta de crédito
- `POST /tokens/nequi` - Tokenizar una cuenta de Nequi
- `GET /tokens/nequi/{token_id}` - Obtener información sobre una Cuenta de Nequi tokenizada
...
#### **Comercios**
- `GET /merchants/{merchantPublicKey}` - Obtener un comercio y token de aceptación

#### **PSE**
- `GET /pse/financial_institutions` - Listado de instituciones financieras de PSE

### **Llaves de Autenticación (Modo Pruebas):**
...
Tu implementación **YA tiene un gateway centralizado** con 3 clientes especializados bien diseñados:

#### **1.1. WompiGateway (Consultas)** - Líneas 13-80
...
- ✅ Circuit breaker con Redis cache
- ✅ Timeout configurable (10 segundos)
- ✅ Manejo de errores con logging estructurado
- ✅ Método `fetch_transaction(reference)` para consultar transacciones
- ✅ Headers con Bearer token authorization

**Métodos disponibles:**
- `fetch_transaction(reference)` - Consulta transacción por ID/reference
- `_circuit_allows()` - Verifica si el circuito permite requests
- `_record_failure()` - Registra fallo y abre circuito si excede límite
- `_record_success()` - Cierra circuito en caso de éxito

#### **1.2. WompiPaymentClient (Creación de Transacciones)** - Líneas 94-191
...
**Características implementadas:**
- ✅ Circuit breaker independiente para pagos
- ✅ Timeout de 15 segundos (mayor que consultas)
- ✅ Método `create_transaction(payload)` genérico
- ✅ Método `resolve_acceptance_token()` con caché (55 minutos)
- ✅ Manejo de errores HTTP >=400 con circuit breaker

**Métodos disponibles:**
- `create_transaction(payload)` - Crea transacción en Wompi
- `resolve_acceptance_token(base_url, public_key)` - Obtiene token de aceptación
- Circuit breaker methods (compartidos con WompiGateway)

#### **1.3. WompiDisbursementClient (Dispersión de Fondos)** - [finances/services.py:33-141](finances/services.py:33-141)
...
**Características implementadas:**
- ✅ Circuit breaker para dispersiones
- ✅ Método `get_available_balance()` - Consulta balance en centavos
- ✅ Método `create_payout(amount)` - Crea dispersión a desarrollador
- ✅ Manejo de múltiples formatos de respuesta (balanceInCents vs balance_in_cents)

**Endpoints utilizados:**
- `GET {base_url}/accounts` - Consulta balance disponible
- `POST {base_url}/transfers` - Crea dispersión/payout
...
✅ **Cumple con la documentación oficial de Wompi (Paso 3)**
...
✅ **Tu implementación sigue exactamente este algoritmo**

**Nota:** Si se usa `expiration-time`, debe agregarse a la concatenación:
```
"<Referencia><Monto><Moneda><FechaExpiracion><SecretoIntegridad>"
```
❌ **FALTA implementar este caso opcional**
...
✅ **Llaves correctas para modo pruebas** según documentación oficial

**Configuración en [studiozens/settings/base.py:401-413](studiozens/settings/base.py:401-413):**
```python
...
### **4. Webhook Handler con Validación de Firma** ([spa/services/payments.py:38-234](spa/services/payments.py:38-234))
...
class WompiWebhookService:
    ...
    def _validate_signature(self):
        ...
        if not hashlib.sha256(self.sent_signature.encode('utf-8')).hexdigest() == hashlib.sha256(calculated_signature.encode('utf-8')).hexdigest():
            logger.error("[PAYMENT-ALERT] Webhook Error: Firma inválida")
            raise ValueError("Firma del webhook inválida. La petición podría ser fraudulenta.")
```

✅ **Implementa validación de seguridad**
⚠️ **PERO no sigue exactamente el algoritmo oficial** (ver Brecha #5)
...
✅ **Estados alineados con Wompi:** PENDING, APPROVED, DECLINED, ERROR
✅ **Soporte para múltiples tipos de pago**
✅ **Almacena `transaction_id` y `raw_response`** para reconciliación
...
class DeveloperCommissionService:
    ...
    def register_commission(cls, payment):
        """Registra comisión por pago exitoso"""
    ...
    def evaluate_payout(cls):
        """Evalúa si procede dispersión automática"""

    @classmethod
    def _attempt_payout(cls, settings_obj, current_debt: Decimal):
        """Ejecuta dispersión usando WompiDisbursementClient"""
```

✅ **Integración completa con Wompi Payouts**
✅ **Manejo de estados:** PENDING, PAID, FAILED_NSF
...
✅ **Normalización automática de decimales**
✅ **Validaciones robustas**
✅ **Trazabilidad con `wompi_transfer_id`**

---

## 🔴 BRECHAS CRÍTICAS vs DOCUMENTACIÓN DE WOMPI

### **BRECHA #1: Falta Implementar Métodos de Pago Específicos**

**Según la documentación oficial de Wompi, los métodos de pago soportados son:**

| Método de Pago | Estado Actual | Implementación Requerida | Prioridad |
|----------------|---------------|--------------------------|-----------|
| **CARD** (Tarjeta) | ⚠️ Soportado parcialmente | Agregar tokenización con `POST /tokens/cards` | 🟡 Media |
| **NEQUI** | ⚠️ Mencionado en tests | Implementar flujo completo con `POST /tokens/nequi` | 🔴 Alta |
| **PSE** | ❌ **NO IMPLEMENTADO** | **CRÍTICO - Es el método más usado en Colombia** | 🔴 **CRÍTICA** |
| **BANCOLOMBIA_TRANSFER** | ❌ **NO IMPLEMENTADO** | Implementar con `async_payment_url` | 🟡 Media |
| **BANCOLOMBIA_QR** | ❌ **NO IMPLEMENTADO** | Implementar con `sandbox_status` | 🟢 Baja |
| **DAVIPLATA** | ❌ **NO IMPLEMENTADO** | Implementar con OTP | 🟡 Media |
| **BNPL Bancolombia** | ❌ **NO IMPLEMENTADO** | Implementar Buy Now Pay Later | 🟢 Baja |
| **Puntos Colombia** | ❌ **NO IMPLEMENTADO** | Implementar programa de puntos | 🟢 Baja |

#### **Evidencia en el código actual:**

**spa/services/payments.py:341-482** - Solo hay cobro recurrente genérico:
```python
@classmethod
def charge_recurrence_token(cls, user, amount, token):
    ...
    payload = {
        ...
        "reference": reference,
        "payment_source_id": payment_source_id,  # ⚠️ ¿Card? ¿Nequi? ¿Daviplata?
        "recurrent": True,
    }
```

❌ **Problema:** No especifica el tipo de método de pago ni maneja flujos específicos

#### **Datos de Prueba en Sandbox (Documentación Oficial):**
...
# Luego usar data.payment_method.async_payment_url para autenticación
```
...
✅ **Sí usa `WompiPaymentClient.create_transaction()`** ([finances/gateway.py:137-164](finances/gateway.py:137-164))

❌ **PERO solo se usa para cobros recurrentes VIP** ([spa/services/payments.py:428-430](spa/services/payments.py:428-430)):
```python
...
try:
    ...
    logger.exception("Error comunicándose con Wompi al intentar renovar VIP")
```
...
- Pagos de órdenes del marketplace
...
### **BRECHA #3: Falta Tokenización de Tarjetas y Nequi**
...
❌ **El backend NO tokeniza** - Depende 100% del Widget de Wompi en el frontend
...
**Problema:** No hay endpoint backend para tokenizar desde el servidor, solo se acepta `payment_source_id` numérico ya existente.

### **BRECHA #4: Parámetros Faltantes en Transacciones**

**Documentación Wompi (Parámetros opcionales):**
...
```json
{
  ...
  // ❌ FALTA: PAYMENT METHOD (para PSE/Nequi/etc)
  "payment_method": {
    "type": "PSE",
    "user_type": 0,
    "user_legal_id_type": "CC",
...
✅ **SÍ tienes modelo Payment con campos básicos**
❌ **FALTAN campos:**
- `tax_in_cents` (VAT, CONSUMPTION)
- `customer_data` (legal_id, legal_id_type)
- `payment_method` (tipo específico, datos del método)
...
**Nota de Wompi (Documentación):**
> "Es importante resaltar que los impuestos enviados en el objeto `taxes` no se sumarán al total de la transacción. Por ejemplo, en una transacción cuyo total (`amount_in_cents`) es de COP$119,000 y cuyo IVA es de COP$19,000, este último monto ya hace parte del total, implicando entonces que: la base sin impuestos ($100,000) + el IVA ($19,000) = el total ($119,000)."

### **BRECHA #5: Validación de Webhooks NO Sigue Algoritmo Oficial**

**Documentación Wompi (Seguridad de Eventos):**
...
# Extraer valores según properties
...
**Tu código actual:** ([spa/services/payments.py:64-87](spa/services/payments.py:64-87))
...
def _validate_signature(self):
    ...
    # ⚠️ PROBLEMA: Usa json.dumps(data) en lugar de properties específicas
    event_body_str = json.dumps(self.data, separators=(',', ':'))
    ...
    # ⚠️ Comparación doble SHA256 innecesaria
    if not hashlib.sha256(self.sent_signature.encode('utf-8')).hexdigest() == hashlib.sha256(calculated_signature.encode('utf-8')).hexdigest():
        logger.error("[PAYMENT-ALERT] Webhook Error: Firma inválida")
        raise ValueError("Firma del webhook inválida. La petición podría ser fraudulenta.")
```
...
1. **NO usa `signature.properties`** - Debería extraer solo los campos especificados en el array `properties`, no todo el objeto `data`
2. **JSON completo vs valores específicos** - Wompi NO serializa todo el JSON, solo concatena valores específicos
3. **Comparación doble SHA256** - Innecesaria, debe comparar directamente los hexdigest

**Riesgo:** Si Wompi cambia el orden de campos en el JSON del webhook, tu validación fallará aunque sea legítima.
...
❌ **Tu código NO implementa este endpoint**

**Necesidad:** Para mostrar lista de bancos al usuario antes de crear transacción PSE.

### **BRECHA #7: Falta Manejo de Async Payment URLs**

**Documentación Wompi (Bancolombia Transfer):**
...
# Paso 2: Obtener URL de autenticación
auth_url = response["data"]["payment_method"]["async_payment_url"]

# Paso 3: Redirigir usuario a auth_url
# El usuario completa autenticación en página de Bancolombia

# Paso 4: Webhook notifica resultado final
```

❌ **Tu código NO maneja `async_payment_url`**
...
### **BRECHA #8: Falta Validación de Expiration Time**
...
❌ **Tu función `build_integrity_signature`** ([finances/gateway.py:82-91](finances/gateway.py:82-91)) **NO acepta `expiration_time`:**

```python
def build_integrity_signature(reference: str, amount_in_cents: int, currency: str) -> str | None:
    # ❌ Falta parámetro expiration_time
    concatenated = f"{reference}{amount_in_cents}{currency}{integrity_key}"
    return hashlib.sha256(concatenated.encode("utf-8")).hexdigest()
```
...
## 📋 ESTADO DE CENTRALIZACIÓN DE PAGOS

### **Módulos con Lógica de Pagos:**

| Módulo | Integración Wompi | Estado Centralización | Duplicación |
|--------|-------------------|-----------------------|-------------|
| **finances** | ✅ Gateway completo (3 clientes) | ✅ **100% centralizado** | ❌ Ninguna |
| **spa** | ⚠️ Usa `finances.gateway` pero tiene lógica propia de webhooks | 🟡 **70% centralizado** | ⚠️ Webhook handler propio |
| **marketplace** | ⚠️ Duplica lógica de obtención de acceptance token | 🔴 **40% centralizado** | 🔴 Duplica `resolve_acceptance_token` |

### **Análisis Detallado de Duplicación:**

#### **1. marketplace/views.py:248-286** - Duplica Obtención de Acceptance Token

```python
# LÍNEA 248-250: Genera reference y firma (✅ CORRECTO)
...
# LÍNEA 252-257: 🔴 DUPLICA lógica de WompiPaymentClient.resolve_acceptance_token
...
# LÍNEA 266-272: 🔴 DUPLICA build_integrity_signature
...
# Generar firma usando función centralizada
...
#### **2. spa/services/payments.py:341-482** - Duplica Creación de Transacciones

```python
# LÍNEA 428-430: 🟡 USA WompiPaymentClient CORRECTAMENTE
client = WompiPaymentClient(base_url=base_url, private_key=private_key)
try:
    response_data, status_code = client.create_transaction(payload)
```

✅ **Esto está bien** - Usa el gateway centralizado

❌ **PERO** solo se usa para cobros recurrentes VIP, no para pagos generales
...
class WompiWebhookService:
    ...
    """
    def _validate_signature(self):
        # ... validación propia (no usa finances.gateway)

    def process_transaction_update(self):
        # ... procesamiento propio
```

⚠️ **Debería estar en `finances` module** para verdadera centralización

---

## 🛠️ PLAN DE ACCIÓN RECOMENDADO

### **Fase 1: Completar Métodos de Pago** (CRÍTICO) 🔴

#### **1.1. Implementar PSE** (Más usado en Colombia)
...
class WompiPaymentClient:
    ...
    def create_pse_transaction(
        ...
        *,
        ...
        if len(payment_description) > 30:
            raise ValueError("payment_description debe tener máximo 30 caracteres")
    ...
    def get_pse_financial_institutions(self):
        ...
        Example:
            [
                {"financial_institution_code": "1", "financial_institution_name": "Banco de Bogotá"},
...
class WompiPaymentClient:
    ...
    def create_daviplata_transaction(
        ...
        *,
        ...
        """
        Crea una transacción Daviplata.
...
#### **1.5. Implementar Tokenización de Tarjetas**
...
### **Fase 2: Centralizar Marketplace** 🟡

**Objetivo:** Eliminar duplicación de código en marketplace/views.py

**Archivos a modificar:**
- [marketplace/views.py](marketplace/views.py) (líneas 248-286)
...
# Usar función centralizada de firma
signature = build_integrity_signature(
    reference=reference,
    amount_in_cents=amount_in_cents,
    currency=getattr(settings, "WOMPI_CURRENCY", "COP"),
...
- ✅ Usa circuit breaker del gateway
- ✅ Reduce líneas de código

### **Fase 3: Corregir Validación de Webhooks** 🟡

**Objetivo:** Implementar algoritmo oficial de Wompi para validación de firmas
...
def _validate_signature(self):
    ...
    # Paso 1: Concatenar valores según properties
    values = []
    ...
    if calculated_checksum.upper() != sent_checksum.upper():
        logger.error(
            "[PAYMENT-ALERT] Webhook Error: Firma inválida. "
...
# ✅ Coincide con signature.checksum
```
...
### **Fase 4: Agregar Parámetros Faltantes** 🟢
...
def build_integrity_signature(
    ...
    # Concatenar según documentación Wompi
    if expiration_time:
        concatenated = f"{reference}{amount_in_cents}{currency}{expiration_time}{integrity_key}"
    else:
        concatenated = f"{reference}{amount_in_cents}{currency}{integrity_key}"
...
**Opción 1: Campos adicionales (más normalizado)**
```python
class Payment(BaseModel):
    # ... campos existentes ...
...
**Opción 2: JSON Field (más flexible)**
```python
class Payment(BaseModel):
    # ... campos existentes ...
...
**Recomendación:** Opción 1 para campos críticos (customer_legal_id, payment_method_type) + Opción 2 para datos adicionales

#### **4.3. Actualizar WompiPaymentClient para Aceptar Parámetros Adicionales**
...
class WompiPaymentClient:
    ...
    def create_transaction(
        ...
        *,
        ...
        """
        Crea una transacción en Wompi con parámetros opcionales extendidos.

        Args:
            payload: Payload base con amount, reference, etc.
            customer_data: Información del pagador {email, full_name, phone_number, legal_id, legal_id_type}
            shipping_address: Dirección de envío {address_line_1, city, country, phone_number, region}
...
### **Fase 5: Mover Webhook Handler a Finances** 🟢

**Objetivo:** Verdadera centralización moviendo WompiWebhookService a finances
...
class WompiWebhookService:
    ...
    Maneja validación de firma y procesamiento de eventos.
    """
    ...
    def _process_transaction_updated(self):
        ...
        NOTA: Esta lógica debería eventualmente moverse aquí desde spa.services
        """
        # Por ahora, delegar a la implementación existente
...
### **Estado Actual vs Documentación Wompi:**

| Aspecto | Implementado | Faltante | Prioridad | Esfuerzo Estimado |
|---------|--------------|----------|-----------|-------------------|
| **Gateway Centralizado** | ✅ 90% | Circuit breaker en disbursement mejorado | 🟢 Baja | 2 horas |
| **Métodos de Pago** | 🔴 20% | PSE, Nequi, Bancolombia, Daviplata, etc. | 🔴 **CRÍTICA** | 2-3 días |
| **Tokenización** | ❌ 0% | POST /tokens/cards, POST /tokens/nequi | 🔴 **CRÍTICA** | 1 día |
| **Webhooks** | 🟡 70% | Algoritmo de firma oficial, centralización | 🟡 Media | 4-6 horas |
| **Parámetros** | 🟡 60% | customer_data, taxes, payment_method, expiration_time | 🟢 Baja | 1 día |
| **Centralización** | 🟡 70% | Eliminar duplicados en marketplace, mover webhooks | 🟡 Media | 4 horas |
| **PSE Institutions** | ❌ 0% | GET /pse/financial_institutions | 🟡 Media | 2 horas |
| **Async Payments** | ❌ 0% | Manejo de async_payment_url | 🟢 Baja | 3-4 horas |

### **Matriz de Priorización:**

```
CRÍTICO (Hacer YA) 🔴
├── PSE Implementation (Colombia usa mayormente PSE)
├── Nequi Implementation (segundo método más popular)
└── Tokenización de Tarjetas (para cobros recurrentes robustos)

ALTA (Próxima Semana) 🟡
├── Corregir validación de webhooks
├── Centralizar marketplace
└── Daviplata Implementation

MEDIA (Próximo Sprint) 🟢
├── Bancolombia Transfer
├── Customer Data y Taxes
...
├── BNPL
├── Puntos Colombia
└── Webhook Handler en finances module
```

### **Recomendación Final:**

**Tu módulo `finances` tiene una base arquitectónica sólida y bien diseñada**, con:
- ✅ Gateway centralizado con 3 clientes especializados
- ✅ Circuit breakers implementados
- ✅ Manejo de errores robusto
- ✅ Firma de integridad correcta
- ✅ Sistema de comisiones funcional

**PERO le faltan las implementaciones específicas de métodos de pago de Wompi**.

Actualmente el gateway solo se usa para:
- ✅ Consultar transacciones existentes
- ✅ Cobros recurrentes VIP genéricos
- ✅ Dispersión de comisiones al desarrollador

**Para cumplir 100% con la documentación de Wompi y ofrecer todos los métodos de pago a tus usuarios, necesitas:**

1. 🔴 **URGENTE - Implementar PSE**
   - Es el método de pago más usado en Colombia (transferencias bancarias)
   - Sin esto, muchos usuarios no podrán pagar
   - Esfuerzo: ~8 horas

2. 🔴 **URGENTE - Implementar Nequi**
   - Segundo método más popular
   - Muy usado por usuarios jóvenes
   - Esfuerzo: ~6 horas

3. 🔴 **URGENTE - Tokenización de Tarjetas**
   - Necesario para cobros recurrentes robustos
   - Mejora experiencia VIP
   - Esfuerzo: ~4 horas

4. 🟡 **IMPORTANTE - Corregir validación de webhooks**
   - Seguridad crítica
   - Evita rechazar webhooks legítimos
   - Esfuerzo: ~4 horas

5. 🟡 **IMPORTANTE - Centralizar marketplace**
   - Elimina duplicación
   - Mejora mantenibilidad
   - Esfuerzo: ~3 horas

6. 🟢 **RECOMENDADO - Agregar customer_data y taxes**
   - Cumplimiento fiscal
   - Mejor trazabilidad
   - Esfuerzo: ~6 horas

**Total Esfuerzo Crítico:** ~18 horas (2-3 días de desarrollo)
**Total Esfuerzo Completo:** ~31 horas (4-5 días de desarrollo)

---

## 📁 ARCHIVOS PRINCIPALES ANALIZADOS

1. **finances/gateway.py** (191 líneas)
   - WompiGateway (consultas)
   - WompiPaymentClient (creación de transacciones)
   - build_integrity_signature()

2. **finances/services.py** (341 líneas)
   - WompiDisbursementClient (dispersión)
   - DeveloperCommissionService (comisiones)

3. **finances/models.py** (74 líneas)
   - CommissionLedger (con validaciones actualizadas)

4. **spa/services/payments.py** (740 líneas)
   - WompiWebhookService (procesamiento de webhooks)
   - PaymentService (lógica de negocio de pagos)

5. **spa/models/payment.py** (212 líneas)
   - Payment (modelo principal)
   - ClientCredit (saldo a favor)
   - WebhookEvent (registro de webhooks)

6. **marketplace/views.py** (líneas 248-286)
   - Duplicación de lógica Wompi

7. **env.example.txt** (líneas 99-112)
   - Configuración de variables Wompi

8. **studiozens/settings/base.py** (líneas 401-425)
   - Configuración centralizada Wompi
